...
sample_dates_1 <- c("2018-02-01", "2018-03-21", "2018-10-05", 
                    "2019-01-01", "2019-02-18") # character strings

class(sample_dates_1) #character, not date
...
class(sample_dates_1) #now it's date
...
sample_dates_3<- as.Date(sample_dates_2, format = "%m-%d-%Y" ) #specify the format argument. ""Y" is looking for 4 digits year.
...
tm1 #add your computer time zone 
...
tm3 <- as.POSIXct("2010-12-01 11:42:03", tz = "GMT") #change the time zone
tm3
...
# --> 2016-01-01 12:00:00 PST #automatically says PST based on the data point (it's in January)
#24 hour time:
...
mloa2 <- read_csv("https://raw.githubusercontent.com/UCD-R-DAVIS/R-DAVIS/main/data/mauna_loa_met_2001_minute.csv",
                  col_types = "cccccccddddddddd") # specify column types.
...
mloa2$datetime_test <- as_datetime(mloa2$datetime, 
                                   tz="America/Los_Angeles", 
                                   format="%Y-%m-%d, %H:%M") #as_datetime is not the tidyverse function
